...
if [ -f "$script_directory/bundles/$state/createFacility.json" ]; then
  ...
  if [ -f "$baseConfig_file" ]; then
    sed -i "s|pathtosite|$site_path|g" "$baseConfig_file"
    sed -i "s|pathtobackend|$backend_path|g" "$baseConfig_file"
    sed -i "s|pathtofhir|$fhirUrl|g" "$baseConfig_file"
    sed -i "s|pathtokibana|$kibanaUrl|g" "$baseConfig_file"
    sed -i "s|pathtoelastic|$elasticUrl|g" "$baseConfig_file"
    sed -i "s|pathtoredis|$redisUrl|g" "$baseConfig_file"
